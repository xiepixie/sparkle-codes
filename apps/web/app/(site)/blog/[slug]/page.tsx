				...
				<main className="min-w-0 lg:col-start-2 lg:pt-12">
					<header className="mb-12 flex flex-col sm:mb-20">
						...
						<div className="flex flex-col gap-8 border-b border-border/40 pb-10 transition-opacity">
							...
							<div className="flex flex-wrap items-center justify-between gap-6">
								...
								<div className="flex items-center gap-4">
									<div
										className="flex h-1.5 w-1.5 rounded-full bg-emerald-500/40 shadow-glow-sm"
										title="Post is live"
									/>
					...
					<footer className="mt-20 flex flex-col items-center gap-12 border-t border-border/40 pt-16 sm:mt-32">
						...
						>
							<ChevronLeft
								size={18}
								className="transition-transform group-hover/footer-btn:-translate-x-1"
							/>
							BACK TO BLOG
						</Link>
...
export async function generateMetadata({ params }: PostPageProps) {
	...
	return {
		...
		description:
			post.description ||
			"Deep dive into modern architecture, AI interaction, and content engineering.",
		openGraph: {
			type: "article",
			title: post.displayTitle || post.title,
			description: post.description || undefined,
...
export async function generateStaticParams() {
	try {
		...
		console.error(
			"[BUILD] generateStaticParams failed, falling back to empty paths to allow build to continue.",
			err,
		);
		return [{ slug: BUILD_PLACEHOLDER_SLUG }];
	}
