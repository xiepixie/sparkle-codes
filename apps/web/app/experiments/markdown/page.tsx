  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      if (e.shiftKey && lastClickedIndex !== null) {
        ...
        pres.forEach((p, i) => {
          if (i >= start && i <= end) p.classList.add('is-selected');
        });
      } else {
    ...
    const handleDoubleClick = (e: MouseEvent) => {
      ...
      if (mathBlock instanceof HTMLElement && mathBlock.dataset.tex) {
        ...
        if (mathBlock.classList.contains('source-mode')) {
          ...
          const tex = mathBlock.getAttribute('data-tex') || '';
          mathBlock.innerHTML = `
            <div class="code-fence-container latex-source !bg-primary/[0.04]">
              <div class="code-fence mockup-code text-left !bg-transparent p-4">
                <pre class="font-mono text-sm text-[var(--md-primary)]"><code>${tex}</code></pre>
              </div>
            </div>
          `;
        } else {
...
function CodeCopyButton() {
  ...
  const handleCopy = () => {
    ...
    const selectedLines = Array.from(container.querySelectorAll('pre.is-selected'));
    const linesToRead = selectedLines.length > 0 
      ? selectedLines 
  ...
  return (
    <button ref={buttonRef} onClick={handleCopy} className="copy-button relative z-20">
      {state === 'copied' ? <Check className="w-3.5 h-3.5 text-green-500" /> : <Copy className="w-3.5 h-3.5" />}
      <span className="text-[10px] uppercase font-bold tracking-wider">
        {state === 'copied' ? 'Copied' : (selectedCount > 0 ? `Copy ${selectedCount} Lines` : 'Copy')}
      </span>
    </button>
  );
}
    ...
    <div className="min-h-screen bg-background text-foreground p-8 md:p-20 font-sans relative overflow-hidden transition-colors duration-500">
      ...
      <div className="max-w-4xl mx-auto mb-16 p-5 rounded-2xl border border-border bg-card/50 backdrop-blur-3xl flex flex-wrap items-center justify-between gap-6 shadow-glow relative z-10 transition-colors duration-500">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black tracking-tight text-foreground">Sentinel V3.3 Showcase</h1>
            <p className="text-[10px] text-muted-foreground uppercase tracking-[0.2em] font-bold">Full Module Validation</p>
          </div>
        </div>
      ...
      <MarkdownRuntime>
        ...
        >
          <section>
            ...
            <p>
              This document serves as a comprehensive visual validation of the Sentinel <code>markdown.css</code> engine. 
              It demonstrates how deep integration with Next.js App Router and dynamic <strong>Light/Dark DOM Variables</strong> 
              creates an unparalleled reading experience.
            </p>

            <blockquote>
              "In a sea of generic documentation, the physics of our digital objects define the weight of our engineering."
              <br/>— Sentinel Core Protocol
            </blockquote>

            <h2>1. Academic Theory Modules</h2>
            <p>
              Callouts are fundamentally redesigned. Instead of basic boxes, they utilize layered glass morphic 
              gradients and automatic CSS counters (`counter-increment`) to maintain structural rigor in scientific writing.
            </p>

            <div className="md-callout" data-type="theory">
              <div className="md-callout-title">Quantum Morphism</div>
            ...
            <div className="md-callout" data-type="warning">
              <div className="md-callout-title">Critical Attention Required</div>
              <div className="md-callout-content">
                Removing the absolute padding values within the CSS custom properties will result in catastrophic layout collapse across nested MDX components.
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <p>
              By leveraging the <code>MarkdownRuntime</code> React wrapper, static Rust HTML is instantly activated. 
              The block below supports <strong>Line Selection</strong>, <strong>Git Diffs</strong>, and <strong>Contextual Blur</strong>.
            </p>
            ...
            <div className="code-fence-container group">
              <div className="code-fence-header">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-mono opacity-60 text-[11px]">pkg/core/engine.rs</span>
                </div>
                <CodeCopyButton />
              </div>

              <div className="code-fence mockup-code !bg-transparent py-6">
                <pre data-prefix="1" className="line-blur"><code><span className="token keyword">use</span> sentinel::core::<span className="token punctuation">&#123;</span>Bloom, Glow<span className="token punctuation">&#125;</span>;</code></pre>
                <pre data-prefix="2"><code></code></pre>
                <pre data-prefix="3" className="line-remove"><code><span className="token keyword">let</span> renderer = <span className="token function">MarkdownRenderer::new</span>(<span className="token string">"v2_engine"</span>);</code></pre>
                <pre data-prefix="4" className="line-add"><code><span className="token keyword">let</span> root_engine = <span className="token function">StarryNightEngine::initialize</span>();</code></pre>
                <pre data-prefix="5" className="line-highlight"><code>root_engine.<span className="token function">inject_runtime_metrics</span>(0.95);</code></pre>
                <pre data-prefix="6"><code></code></pre>
                <pre data-prefix="7" className="line-blur"><code><span className="token keyword">return</span> <span className="token keyword">Ok</span>(root_engine);</code></pre>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <p>
              Standard markdown tables have been revolutionized into <strong>Glass Zebra</strong> elements. 
              They adapt flawlessly to <code>COMPACT</code> mode, ensuring maximum data legibility on small screens.
            </p>
            ...
            <div className="md-callout" data-type="example">
              <div className="md-callout-title">Implementation Checklist</div>
              <div className="md-callout-content">
                To achieve the table metrics shown above, ensure the following steps are met:
                <ul>
                  <li>Verify <code>theme.css</code> OKLCH variables are present in the DOM.</li>
                  <li>Confirm the <code>&lt;MarkdownRuntime&gt;</code> is mounted at layout root.</li>
                  <li>Do NOT use arbitrary Tailwind utility classes within Markdown text strings.</li>
                </ul>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <p>
              Mathematical formulas undergo a complete paradigm shift. Try <strong>double-clicking</strong> the equation 
              below to instantly demystify the rendered calculus back into its raw, copyable LaTeX source block.
            </p>
