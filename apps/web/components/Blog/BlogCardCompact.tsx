		...
		>
			<div className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-border/40 transition-all duration-300 ease-[cubic-bezier(0.2,1,0.2,1)] hover:-translate-y-px [transform-style:preserve-3d] [backface-visibility:hidden] transform-gpu isolation-isolate">
				...
				<div className="absolute inset-0 pointer-events-none z-20 overflow-hidden rounded-[inherit]">
					<svg className="absolute inset-0 h-full w-full" style={{ overflow: "visible" }} aria-hidden="true">
						<motion.rect
							...
							transition={{ 
								pathLength: { 
									duration: prefetchState === "ready" ? (hasBeenPrefetched ? 0.15 : 0.25) : 1.0,
							...
							style={{ 
								filter: (prefetchState === "ready" || (prefetchState === "idle" && hasBeenPrefetched))
									? (prefetchState === "ready" ? "drop-shadow(0 0 6px var(--color-primary))" : "drop-shadow(0 0 3px var(--color-primary))")
									: "none"
							}}
						/>
					</svg>
				...
				<div className="absolute bottom-5 right-5 sm:bottom-6 sm:right-6 z-30 pointer-events-none">
					<div className={`flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-full border transition-all duration-400 ease-[cubic-bezier(0.16,1,0.3,1)]
						${prefetchState === "ready" 
					...
					`}>
						<ArrowRight className={`h-2.5 w-2.5 sm:h-3 sm:w-3 transition-all duration-500 
